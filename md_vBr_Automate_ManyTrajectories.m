...
for numRuns = 1:1000
    ...
    trajectoryNum = 1;
    %     fileName_PDB = strcat('vBr_NN-G98_',num2str(trajectoryNum),'_PDB.pdb');
        fileName_PDB = strcat('vBr_NN-G98_',num2str(numRuns),'_PDB.pdb');

    fid=fopen(fileName_PDB,'w');

[coord, vel] = automate_md();
...
for count=1:totalITERATION
    ...
    [in,coord,vel,accelrn,PE,KE] = velverlet_compPhys(count,coord,vel,accelrn,total,1,1,mass);

    if( (in(1,1)>10) | (in(2,1)>10) | (in(3,1)>10) )
        
        if( (in(1,1) > 5) & (in(3,1) > 5) )
            reaction{numRuns}= 'H1Br';
            
        elseif( (in(1,1) > 5) & (in(2,1) > 5) ) 
            reaction{numRuns}= 'H2Br';
            
        elseif( (in(2,1) > 5) & (in(3,1) > 5) )
            reaction{numRuns}= 'H2';
